...
const HajjForm = () => {
    ...
    return (
        <div className="w-full max-w-4xl flex justify-center items-center bg-gray-100 p-2 sm:p-4 hajj-form-wrapper">
                                ...
                                {submissions.length > 0 ? (
                                    submissions.map((sub) => (
                                        <tr key={sub.id} className={`border-b ${editingId === sub.id ? 'bg-yellow-100' : 'hover:bg-gray-50'}`}>
                                            <td className="py-3 px-4 text-sm">{`${sub.firstName} ${sub.lastName}`}</td>
